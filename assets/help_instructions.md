...
At the opening screen, enter your PROJECT NAME, your number of factors, name your FACTOR(s), and the two states of each factor.  Choose your 𝛂 risk and the sample size that provides the appropriate level of test resolution.  Then click SUBMIT.  A data matrix will then appear.  Shoot the appropriate two-shot groups and enter the size of the groups in the GROUP SIZE column.  Note: You can pair shot 1 and shot 2, then shot 3 and shot 4.  You cannot pair shot 2 and shot 3.  The pairs must be independent.

When all the group sizes have been entered, click SHOW RESULTS.  You’ll see a chart with a centerline and upper and lower decision limits.  If the average of a state is outside the decision limit, it is different from the average of all the data at your chosen level of risk or better.

If you have a setup that you already like, it is a good idea to shoot a set of two-shot groups with it, so you have a basis for comparison.
