...
3. **Enter range data** — The app generates a data matrix pairing every factor-state combination. Shoot the prescribed two-shot groups and enter each group's size in the **Ranges** column. Run order can be randomized to guard against drift. Copy/paste, undo/redo, and fraction input (`1/4` → `0.25`) are supported.
...
- **Spreadsheet-style data entry** — Enter/Tab advance through the Ranges column, values commit on every keystroke, grid-level undo/redo (Cmd/Ctrl+Z), and clipboard support.
